...
W044 = 20260115000000
W059 = 20250223000000
W047 = 20260206000001   # PHerc0139 w047 (larger wing patch, 9.362um)
CKPT = "models/double/v14_mil_deep_triple_v3_final.pth"
...
ARGS: Dict[str, Any] = {
    "scroll-ids":         f"{W044},{W059},{W047}",  # triple-scroll merged stream
    "tile-size":          16,                 # v2: shrink 32->16 (9.4um voxels are coarse;
                                              #     16 divides the 32-wide chunk cleanly -> finer grid,
                                              #     no chunk-straddle waste. v14_mil_deep 16->8 after pool)
    "train-d-start":      0,
    "train-d-end":        28,
    "d-start":            0,
    "d-end":              28,
    ...
    "crop-x-frac":        "0.0,1.0",
    "crop-y-frac":        "0.0,1.0",
    # global split = w044's: horizontal (y) 80.55/19.45. w059 overridden below to vertical.
    "split-axis":         "y",
    "train-split-frac":   0.8055,
    # per-user: w059 and w047 split VERTICALLY (left 75% train / right 25% valid).
    # w044 keeps the global horizontal split. w059's narrow horizontal band has ~0 valid
    # tiles under horizontal split; vertical gives a clean column. w047 same convention.
    "split-override":     [f"{W059}:x:0.75", f"{W047}:x:0.75"],
    "batch-size":         64,                 # v14_mil_deep memory footprint
    "lr":                 2e-4,
    "l1-lambda":          3e-7,               # v2: halved 6e-7 -> 3e-7 (2nd fragment stretches model)
    "conv1-drop":         0.05,               # v2: halved 0.10 -> 0.05
    "conv2-drop":         0.075,              # v2: halved 0.15 -> 0.075 (very last before voxel head)
    "data-aug":           0,
    "num-workers":        0,                  # 0 = main-process loading (most stable; reduced after HW crashes)
    "mask-memmap":        True,
    "no-hard-mining":     True,
    "no-probe-rois":      True,
    "arch":               "v14_mil_deep",
...
def dict_to_cli_args(d: Dict[str, Any]) -> List[str]:
    ...
    for k, v in d.items():
        if isinstance(v, bool):
            ...
            # repeatable flags: emit one --flag value per list element
            for item in v:
...
def main():
    ...
    exp_name = "cmp_p0139_triple_v14_mil_deep_v3"
    cmd = [args.python_exe, "train.py", "-n", exp_name, "--log-dir", str(runs_dir)]
    cmd += dict_to_cli_args(ARGS)

    print("\n" + "=" * 78)
    print("[triple] PHerc0139 w044 + w059 + w047 -- v14_mil_deep, triple-scroll v3")
    print(f"         epochs=20 eval-int=20 tile=16 l1=3e-7 drop=(0.05,0.075) batch=64")
    print(f"         w059+w047 split=x/0.75 (vertical), w044 split=y/0.8055 (horizontal)")
    print("=" * 78)
    print(f"   scrolls: {W044} (w044) + {W059} (w059) + {W047} (w047, larger wing patch)")
