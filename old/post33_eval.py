...
def render(args) -> int:
    ...
    label_dir = ROOT / (run_data.get("inklabel_dir") or "./eroded_inklabels")
    ...
    if args.notebook == "eval":
        ...
        for path in sorted(ZARR_DIR.glob("*.zarr")):
            sid = path.stem
            if sid.isdigit() and (label_dir / f"{sid}.png").is_file() and int(sid) not in known:
        ...
        for name, sid in scrolls.items():
            ...
            missing = [
                label for label, ok in (
                    ...
                    ("label", (label_dir / f"{sid}.png").is_file()),
        ...
        for path in sorted(ZARR_DIR.glob("*.zarr")):
            sid = path.stem
            if sid.isdigit() and not (label_dir / f"{sid}.png").is_file() and int(sid) not in fragments.values():
