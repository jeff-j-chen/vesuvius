...
def _base_config(exp_name: str) -> Config:
    ...
    c.tra.eval_int  = 20
    c.tra.test_int  = 9999
    # do NOT load test frags: test_int=9999 means test figures never fire in these 20ep
    # runs, but loading them (incl. the 7GB 15921^2 PHerc1203) eats ~12GB RAM and caused
    # OOM hard-crashes when assembly ran concurrently. empty list = skip the load.
...
TESTS = [
    # WEAKENED AGAIN (2026-07-21): prior combo (a06/a05 + hdrop0.4-0.5 + cutout0.5/0.35 +
    # L1 3-7e-5) still suppressed learning too much. halve intensities/probs again, soften
    # cutout, drop head_drop, lower L1. flip stays high (lossless symmetry).
    # t01: moderate-weak
    dict(tid="t01",
         ...
         l1=3e-5,
         tag="weak_hdrop03_cutout025_L1=3e-5"),

    # t02: very-weak (barely any regularization beyond flip + slight jitter)
    dict(tid="t02",
         ...
         l1=2e-5,
         tag="veryweak_hdrop02_cutout015_L1=2e-5"),
]
