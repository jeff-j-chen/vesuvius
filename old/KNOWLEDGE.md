...
## 21) Campaign 25 Baseline and Campaign 26 Study (2026-09-16)

Campaign 25 established an 11-fragment baseline across seven physical scrolls. Sampling is
round-robin by physical scroll, with PHerc0139 repeated four times per cycle and every other
scroll sampled once. The baseline uses character-balanced sampling, literal surface-relative
depth input, a 192px context, a c64/t16 multitile target, BCE, and nine epochs. DANN and SupCon
are disabled. Per-scroll train and validation PR-AUC remain enabled.

Campaign 26 asks which inductive biases improve transfer between physical scrolls while keeping
that data and sampling baseline fixed. Its arms fall into four groups:

1. Large-kernel features: zero-initialized MedNeXt adapters with spatial kernels 5 and 7, plus
  kernel-7 MedNeXt combined with divided attention.
2. Domain generalization: class-conditional prototype alignment, conditional CORAL, CDAN, MLDG
  with PHerc1667 or PHerc0139 held out, strong MixStyle, SagNet, a local-64 branch, continuous
  depth-profile FiLM, and masked-MAE continuation.
3. Normalization and pooling: the shallow-IBN baseline, full-network IBN, BatchNorm, asymmetric
  WELDON, multi-k WELDON, and depth-supported WELDON.
4. Dimensionality: a wider early 3D-to-2D model, mid-network 3D-to-2D collapse, factorized
  `(2+1)D` blocks, and a learned depth-attention 2D output head.

The expanded matrix adds 15 one-variable sensitivity arms: weak/strong 0.03/0.3 coefficients for
prototype alignment, CORAL, CDAN, SagNet, and masked-MAE continuation; MixStyle probability 0.4
and alpha 0.3; dual-scale fusion weights 0.10 and 0.50; and a 128-wide style-FiLM hidden layer.
The baseline alone renders a full w044 figure at epoch 9; every other arm keeps `eval_int=999`.

Campaign 26 requires matched 2,000-step MAE adaptation for full-network IBN, BatchNorm, the wider
early 3D-to-2D path, the mid 3D-to-2D path, and factorized `(2+1)D` blocks. Each adaptation starts
from the Campaign 25 base checkpoint, freezes compatible restored tensors, and trains the new
architecture-specific path. Four unlabeled test zarrs were removed for disk space, so preflight
uses the 24 intended corpus zarrs still present and records the four omitted IDs in each completion
marker. Exact MedNeXt variants retain their matching Campaign 25 checkpoints. The depth-attention
output head is absent from the MAE reconstruction path, so that arm uses the fully pretrained 3D
backbone and learns only its small new output head during supervised training.

Most arms disable periodic full-scroll figures to control runtime. The baseline renders w044 at
epoch 9, while every arm records per-scroll metrics. Logs are written to `runs_archs26/` and final
checkpoints to `models/archs26/<arm>/final.pth`.

## 22) Campaign 27: Winner Combinations and Domain Compatibility (2026-09-17)

Campaign 27 promotes the nine-epoch full-IBN Campaign 26 run as its external baseline. Its complete
run directory is copied into `runs_archs27/26_pure_ibn_17_07-19-08`; the runner never queues a
baseline arm. Every runnable arm uses full IBN and matched MAE initialization.

Campaign 26 produced three repeatable but domain-concentrated directions: dual-scale local64/mix
0.50 modestly improved aggregate validation, mid-3D/2D especially improved PHerc0500P2, and physical
domain GroupDRO raised PHerc0009B and PHerc0500P2 while hurting PHerc0139. Gated cue stems showed the
same 0009B/0500P2 concentration. No method materially raised every domain, indicating conflicting
domain gradients rather than one missing universal architectural feature.

Campaign 27 first retests WELDON k4, dual-scale local64/mix0.50, and their combination. Repaired
MixStyle and SagNet arms use float32 Beta sampling under autocast. Mid-3D/2D receives matched
full-IBN MAE adaptation. Rotating MLDG uses beta 0.25 so the held domain regularizes each update
without becoming half the objective; a dual-scale combination tests whether local evidence makes
that meta-objective less destructive.

Domain-gradient arms compute per-physical-domain loss gradients on the final decoder and active
prediction heads. An EMA cosine matrix is logged to TensorBoard. Complete-link clusters require
every pair in a cluster to exceed cosine 0.8. `cluster_balance` weights compatibility clusters
equally, `cluster_worst` optimizes the highest-loss cluster, and `conflict_weighted` strongly
upweights domains whose gradients fall below the compatibility threshold. These objectives measure
task-gradient conflict directly rather than forcing domain embeddings to look alike.

Selected combinations are intentionally sparse:

- WELDON plus dual-scale: global robust evidence pooling plus an independent local score.
- gated stems plus dual-scale: adaptive raw/LCN/dZ evidence in the global branch plus an ungated
  local expert.
- physical GroupDRO plus dual-scale: persistent worst-domain weighting plus stronger local evidence.
- mid-3D/2D plus physical GroupDRO: the strongest 0500P2 architecture plus an external domain loss.
- mid-3D/2D plus gated stems: combine the 0500P2-sensitive dimensionality change with the cue
  selector that raised the 0009B/0500P2 floor.
- mid-3D/2D plus dual-scale: retain the sheet-plane decoder while adding an independent local 3D
  expert; a final version also adds gated stems and physical GroupDRO.
- adaptive dual-scale routing starts at the proven fixed 0.50 local contribution, then predicts a
  per-sample local weight from continuous depth-profile mean/std statistics. This can vary by an
  unseen scroll without requiring a known domain ID.
- final stacks combine WELDON, gated stems, dual-scale, and either GroupDRO or gradient-conflict
  weighting. The two robust objectives are alternatives, not simultaneous losses.

Compatibility rules are enforced. Mid-3D/2D now supports gated stems before its volumetric encoder
and an independent local 3D expert after its 2D score. WELDON, MixStyle, SagNet, and sparse deep
supervision still require bespoke 2D-path definitions and remain disabled rather than silently
bypassed. They are implementation limitations, not mathematical impossibilities, but their weak or
unknown individual evidence does not justify additional branches yet. MLDG uses a separate bilevel
optimizer path; combining it with GroupDRO, CVaR, SagNet, MAE continuation, or domain-gradient
weighting requires defining a new meta-objective rather than toggling two losses. Domain-gradient
