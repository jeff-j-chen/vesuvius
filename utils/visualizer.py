...
def _patch_character_f1_layout(scrolls):
    """build one train/valid multiline chart for each individual patch."""
    return {
        f"patch_{patch_id}": [
            "Multiline",
            [
                f"Character/Patch/{patch_id}/F1/Train",
                f"Character/Patch/{patch_id}/F1/Valid",
            ],
        ]
        for patch_id in dict.fromkeys(int(scroll.scroll_id) for scroll in scrolls)
    }
...
def _process_chunk(valid, pm_sum, pm_cnt, pm_sum_t, pm_cnt_t, model, device,
                   tile, out_tile, mt_n, h_small, w_small, tf, tf_inv,
                   infer_bs, also_tta, tta):
    """process windows that each emit an (n,n) sub-tile grid that is
...
def predict_tiles(config, model, vol, mask, coords, y_range, x_range, depth_start, volume_name, g_mean, g_std, g_min, g_max, tta=False, also_tta=False, surface_depth_map=None, surface_confidence_map=None, surface_depth_offset=0):
    ...
    tta: when True, average the model output over spatial transforms after undoing
    each transform on the multitile output grid.
    """
    from collections import defaultdict
    ...
    if not bool(getattr(config.model, "multitile", True)):
        raise ValueError("predict_tiles requires multitile model outputs")
    mt_n = int(getattr(config.model, "multitile_grid", 4))
    out_tile = int(getattr(config.model, "multitile_subtile", 8))
    h_small = H // out_tile
    w_small = W // out_tile
    ...
    def _read_row(y_off):
        ...
        if surface_relative_window:
            ...
            for x_off in x_offs:
                ...
                sy0 = y_abs - pad
                sx0 = x_abs - pad
                target_size = min(mt_n * out_tile, sp)
                target_y = y_abs + (tile - target_size) // 2
                target_x = x_abs + (tile - target_size) // 2
    ...
    def _flush():
        ...
        _process_chunk(valid, pm_sum, pm_cnt, pm_sum_t, pm_cnt_t, model, device,
    ...
    # average the overlapping window contributions per sub-tile cell
...
class TensorboardVisualizer:
    def __init__(self, config: Config, mode: str = 'train', scroll_id=None, log_suffix: str = None,
        ...
        self.layout = {
            ...
            "Patch_Character_F1": _patch_character_f1_layout(config.data.scrolls),
        }
    ...
    def load_visualization_volume(self) -> bool:
        ...
        if isinstance(self.volume, np.ndarray):
            return False
        print(
            f"[vis-preload] scroll {self.scroll1_id}: loading full volume into RAM",
            flush=True,
        )
        streamed = self.volume
        source = streamed
        if bool(getattr(streamed, "normalized", False)) and hasattr(streamed, "path"):
            import zarr

            source = zarr.open(streamed.path, mode="r")
        loaded = np.ascontiguousarray(source[:, :, :])
        loaded.setflags(write=False)
        self._streamed_volume = streamed
        self.volume = loaded
        print(
            f"[vis-preload] scroll {self.scroll1_id}: ready "
            f"{loaded.nbytes / 1024**3:.2f} GiB",
            flush=True,
        )
        return True
    ...
    def release_visualization_volume(self) -> None:
        ...
        if streamed is None:
            return
        self.volume = streamed
        self._streamed_volume = None
        gc.collect()
    ...
    def _find_dense_probe_region(self, probe_px=512):
        """stub -- superseded by _build_probe_specs from probe_rois.json."""
        return 0, 0, probe_px
    ...
    def add_dense_probe_figure(self, epoch, model):
        """stub -- dense probe mode removed; use add_probe_region_figures instead."""
    ...
    def log_epoch_metrics(self, epoch, model, train_metrics, val_metrics, learning_rate, time_elapsed, params, pos_weight):
        ...
        self.log_patch_character_f1(epoch, train_metrics, val_metrics)
    ...
    def log_patch_character_f1(self, epoch, train_metrics, val_metrics):
        """log paired character f1 series for each individual patch."""
        train_values = train_metrics.get("per_patch_character_f1", {})
        valid_values = val_metrics.get("per_patch_character_f1", {})
        patch_ids = dict.fromkeys([*train_values, *valid_values])
        for patch_id in patch_ids:
            if patch_id in train_values:
                self.writer.add_scalar(
                    f"Character/Patch/{patch_id}/F1/Train",
                    float(train_values[patch_id]),
                    epoch,
                )
            if patch_id in valid_values:
                self.writer.add_scalar(
                    f"Character/Patch/{patch_id}/F1/Valid",
                    float(valid_values[patch_id]),
                    epoch,
                )
    ...
    def add_evaluation_figures(self, epoch, model):
        ...
        if all_pred_data:
            ...
            # label maps and split indices use the multitile output stride
            _out_tile = int(getattr(self.c.model, "multitile_subtile", 8))
    ...
    def add_dense_evaluation_figure(self, epoch, model):
        """stub -- dense pixel-level eval removed; use add_evaluation_figures instead."""
    ...
    def _collect_probe_region_predictions(self, model, spec):
        ...
        # build label maps at the multitile output stride
        _out_tile = int(getattr(self.c.model, "multitile_subtile", 8))
