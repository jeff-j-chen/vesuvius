...
TESTS = [
    ...
    # --- AdamW weight-decay sweep, shifted UP to actually BITE. at lr=1e-4 over ~20k steps the
    #     effective pull ~ 1-exp(-lr*wd*N) ~ 1-exp(-2*wd): 1e-3~0.2% (running baseline), 1e-2~2%,
    #     3e-2~6%, 1e-1~18%, 3e-1~45%, 5e-1~63%, 1e0~86%. l1 DROPPED everywhere (proven inert in
    #     Adam -- it's a per-weight threshold, not a dial); l1+wd combos removed. ---
    # _mk48("wd1e3", "ctx48_wd1e3_nol1", weight_decay=1e-3, l1=0.0),   # running -- clean low-reg baseline
    # _mk48("wd1e1", "ctx48_wd1e1_nol1", weight_decay=1e-1, l1=0.0),
    _mk48("wd3e1", "ctx48_wd3e1_nol1", weight_decay=3e-1, l1=0.0),
    _mk48("wd5e1", "ctx48_wd5e1_nol1", weight_decay=5e-1, l1=0.0),
    _mk48("wd1e0", "ctx48_wd1e0_nol1", weight_decay=1e0,  l1=0.0),
]
