...
class NnUnetMAE(nn.Module):
    ...
    def __init__(self, backbone, depth: int):
        ...
        self.mid_2d = bool(getattr(backbone, "_mid_2d_unet", False))
        if self.early_2d:
            decoder_channels = int(backbone.early2d_head.in_channels)
        elif self.mid_2d:
            decoder_channels = int(backbone.mid2d_head.in_channels)
        else:
        ...
        self.recon_head = (
            ...
            if self.early_2d or self.mid_2d
            else nn.Conv3d(decoder_channels, 1, kernel_size=1, bias=True)
        )
...
def main():
    ...
    ap.add_argument("--norm-mode", choices=("instance", "ibn", "ibn_full", "batch"), default=None)
    ...
    ap.add_argument("--two-d-strided-down", action="store_true")
    ...
    ap.add_argument("--planar-early-convs", action="store_true")
    ...
    cfg.model.two_d_strided_down = bool(args.two_d_strided_down)
    ...
    cfg.model.planar_early_convs = bool(args.planar_early_convs)
    ...
    if args.init_weights:
        ...
        compatible = {key: cleaned[key] for key in loaded_keys}
        incompatible = model.backbone.load_state_dict(compatible, strict=False)
        print(
            f"[mae] warm-started backbone from {args.init_weights}: "
            f"loaded={len(compatible)} missing={len(incompatible.missing_keys)} "
            f"shape-or-key-skipped={len(cleaned) - len(compatible)}"
        )
    ...
    if args.steps > 0:
        print(f"[mae] nnunet3d backbone  ctx={args.ctx} ds={args.ds} depth={args.depth} "
            ...
            f"[mae] architecture norm={cfg.model.norm_mode} "
            f"early_2d=({args.early_2d_unet},x{args.early_2d_channels_mult}) "
            f"mid_2d={args.mid_2d_unet} factorized_2plus1d={args.factorized_2plus1d} "
