...
Run a model on every single test scroll. 9 test patches correspond to 9 test scrolls; each was generated by me. Generates a figure that will require some slight edits before being able to be submitted for a prize.
...
The nine VC3D-grown test surfaces are the final discovery targets. They come from physical scrolls outside the supervised corpus.
...
| `20260921094413` | PHerc0846A | `(28, 5021, 7721)`* | pending | 22.88 cm² | `auto_grown_20260921094413486` |
| `20260918132724` | PHerc0175A | `(28, 4501, 5141)`* | pending | 11.95 cm² | `auto_grown_20260918132724424` |
| `20260922073234` | PHerc0306B | `(28, 5121, 6181)`* | pending | 14.13 cm² | `auto_grown_20260922073234974` |
| `20260922161631` | PHerc0800 | `(28, 6921, 8081)`* | pending | ~23.36 cm² | merged `auto_grown_20260922161631422` |

\* Expected shape from the cropped tifxyz grid; not yet assembled, so mask coverage and surface labels are pending.

PHerc1447, PHerc0175A, PHerc0306B, and PHerc0800 were rendered from 8.640µm / 116keV scans. The other five use 9.362µm / 113keV scans. `assemble_test_segments.py` renders each surface from its tifxyz coordinates and raw CT volume.

After assembly, build the literal surface maps for the four newest test surfaces (the other test surfaces already have them). `test_inference.ipynb` performs every step below automatically for any fragment missing its zarr, normalization entry, or surface maps; the commands are only needed to prepare data ahead of time.

```bash
python3 assemble_test_segments.py --only 20260921094413 --only 20260918132724 --only 20260922073234 --only 20260922161631
python3 precompute_norm.py --scroll-id 20260921094413 --scroll-id 20260918132724 --scroll-id 20260922073234 --scroll-id 20260922161631
for id in 20260921094413 20260918132724 20260922073234 20260922161631; do
	python3 generate_surface_supervision.py --scroll-id "$id" --z-start 4 --z-end 28
done
```
