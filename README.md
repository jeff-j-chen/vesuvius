...
# optional: assemble the eight test fragments
...
Run a model on every single test scroll. 8 test patches correspond to 8 test scrolls; each was generated by me. Generates a figure that will require some slight edits before being able to be submitted for a prize.
...
The eight VC3D-grown test surfaces are the final discovery targets. They come from physical scrolls outside the supervised corpus.
...
PHerc1447, PHerc0175A, and PHerc0306B were rendered from 8.640µm / 116keV scans. The other five use 9.362µm / 113keV scans. `assemble_test_segments.py` renders each surface from its tifxyz coordinates and raw CT volume.

After assembly, build the literal surface maps for the three newest test surfaces (the other test surfaces already have them):

```bash
python3 assemble_test_segments.py --only 20260921094413 --only 20260918132724 --only 20260922073234
python3 precompute_norm.py --scroll-id 20260921094413 --scroll-id 20260918132724 --scroll-id 20260922073234
for id in 20260921094413 20260918132724 20260922073234; do
...
| `assemble_test_segments.py` | Renders the eight unlabeled test surfaces |
...
| `test_inference.ipynb` | Searches the eight prize-target test scrolls for readable letters |
